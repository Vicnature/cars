...
export default function PaymentInstructionsPage() {
		...
		const fetchInstructions = async () => {
			try {
				const data = await sanityClient.fetch(
					`*[_type == "paymentInstructions"][0]{_id, instructions}`,
				);
				if (data) {
					setDocId(data._id);
					setInstructions(
						data.instructions
							?.map((b: PortableTextBlock) => b.children?.[0]?.text)
							.join("\n") || "",
					);
				}
			} catch (err) {
	...
	const handleSave = async () => {
		...
		try {
			...
			alert("Success: Payment instructions updated.");
		} catch (err) {
			alert("Error Failed to update instructions.");
		} finally {
			setSaving(false);
		}
	};
	...
	const handleDelete = async () => {
		...
		const confirmDelete = confirm(
			"Are you sure you want to delete the instructions?",
		);
